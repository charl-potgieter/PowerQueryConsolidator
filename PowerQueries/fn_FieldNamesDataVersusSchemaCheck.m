...
let
    ...
    CustomFunctionType = type function (
        ...
        DirectionToCheck as (type text meta [Documentation.AllowedValues = {"Fields in data not in schema", "Fields in schema not in data"}] )
        )
        as table meta [
            Documentation.Name =  "fn_FieldNamesDataVersusSchemaCheck", 
            Documentation.LongDescription = "Checks for differences between schema and data field names", 
    ...
    ReturnValue = Value.ReplaceType(fn_Raw, CustomFunctionType)
